class CueHeaders
  ...
  def parse_performer
    ...
    @headers_arr.each do |line|
      if line =~ /^#{prefix}.*/i
        self.performer = line[/(?:#{prefix})(.*)/, 1].gsub(/^['"]|['"]$/,'')
        @headers_arr.delete(line)
        break
      end
    end
  ...
  def parse_comment
    ...
    @headers_arr.each do |line|
      if line =~ /^#{prefix}.*/i
        self.comment = line[/(?:#{prefix})(.*)/, 1].gsub(/^['"]|['"]$/,'')
        @headers_arr.delete(line)
        break
      end
    end
  ...
  def parse_composer
    ...
    @headers_arr.each do |line|
      if line =~ /^#{prefix}.*/i
        self.composer = line[/(?:#{prefix})(.*)/, 1].gsub(/^['"]|['"]$/,'')
        @headers_arr.delete(line)
        break
      end
    end
  ...
  def parse_file
    ...
    @headers_arr.each do |line|
      if line =~ /^#{prefix}.*WAVE/i
        self.file = line[/(?:#{prefix})(.*)\s+WAVE/, 1].gsub(/^['"]|['"]$/,'')
        @headers_arr.delete(line)
        break
      end
    end
